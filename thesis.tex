...
\linespread{1.5} 
\newcommand{\sysname}{RetroWrite\_ARM\xspace}
\newcommand{\sysnameold}{RetroWrite\_x64\xspace}
...
possessing its source code in such a way that the binary under 
investigation keeps the same functionality.

At first, binary rewriting was used to change parts of a program during execution
(run-time patching, with the first applications dating back to the 60s~\cite{miller2006binary})
but today it evolved into a stable and researched field of computer science, with 
a number of applications ranging from security to software optimization.
...
\begin{itemize}
	...
	\item \textbf{Hardening}: This is by far the most widespread use case of
		...
		tools that make recompilation with hardening passes enabled impossible. 
...
a generic assembler to produce the instrumented executable. 
