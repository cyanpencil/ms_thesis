...
example, on Android the average phone is outdated by \todo{XXX}. Security 
testing in mobile environments is extremely challenging as vendors often 
release components as binary-only, i.e., without accompanying source code.  
Binaries cannot be readily evaluated by security analysts, resulting in 
potentially undiscovered vulnerabilities in this binary-only modules, which 
often run with high privileges and are exposed to the network.  Binary 
rewriting allows analysts to work with binary-only modules but existing 
solutions are faced with severe disadvantages of either high overhead for 
dynamic rewriting and limited rewriting capabilities for static solutions.
...
however it is also true that \texttt{ARM} poses very different challenges in 
the realm of static rewriting. Trampolines and other kinds of instrumentation 
are made much easier by the fixed-size instruction set, as problems such as 
overwriting instructions shorter than a jump are not present.  However, that's 
not always true: the modern \emph{symbolization} approach for instrumentation 
that \sysname and other recent binary rewriters adopted becomes particularly 
harder. 
...
%Mobile devices are increasingly becoming the subject of auditing and security
%analysis as the mobile phone market rises each year. Countless tools have been
%developed to help or even automatize the vulnerability researcher's role;
%however, most of those tools are targeted towards open source software.

%Unfortunately, in spite of the increasing popularity of open source software,
%closed source modules and binaries are still much more common, especially in
%the phone market: while 75\% \cite{statista} of mobile users use Android or
...
%As an example, one of the most efficient techniques for automatic vulnerability
%discovery, \emph{fuzzing}, is really effective only when applied on software
%accompanied by its source code.  When considering fuzzing closed source
%binaries, the main difficulties that arise are due to the fact that the
%compilation process discards a large amount of information that could be very
%useful while hunting bugs, such as type information, the control flow graph,
%array boundaries, distinction between scalars/references, and much more.
%Still, there are techniques that can help instrument a binary and fuzz it in
%those conditions, and they all fall into a common category called \emph{Binary
%rewriting}.
...
%Still, inspecting the code is not the only way of auditing a codebase, in 
...
%of this approach is that at runtime the rewriter can recover some of the 
...
We will now go over the goals that that we put oursevles at the start of the 
project, explain which were the key issues that we had to face due to the 
quirks of the ARM architecture, and the solutions we adopted to overcome those 
problems both in the \emph{symbolization} and in the \emph{instrumentation} 
sides of \sysname.
...
instrumentaion.  The symbolizer takes care of loading and analyzing a binary, 
